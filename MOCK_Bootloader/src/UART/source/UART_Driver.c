...
#define NULL 			0
#define LPUART0_IRQn_ID 31U  /* Interrupt number of LPUART0 */
...
static void UART0_ClockEnable(void)
{
    ...
    IP_PCC->PCCn[PCC_LPUART0_INDEX] |= PCC_PCCn_PCS(1) | PCC_PCCn_CGC_MASK; /* PCS=2 SPLL_DIV2_CLK */
}
...
static int32_t UART0_Control(uint32_t control, uint32_t arg)
{
    switch (control & ARM_USART_CONTROL_Msk)
    {
        case ARM_USART_MODE_ASYNCHRONOUS:
        {
            /* Configure baudrate */
            uint32_t baud = arg;
            uint32_t clk = 8000000; /* 8 MHz source */
            uint8_t osr = 15u;		/*15 + 1 */
            uint16_t sbr = (uint16_t)(clk / (baud * (osr+1)));
            IP_LPUART0->BAUD = LPUART_BAUD_SBR(sbr) | LPUART_BAUD_OSR(osr);
...
void LPUART0_RxTx_IRQHandler(void)
{
    ...
    uint32_t crl 	= IP_LPUART0->CTRL;
    /* Transmit interrupt */
    if ((status & LPUART_STAT_TDRE_MASK) && (crl & LPUART_CTRL_TIE_MASK))
    {
        if (tx_cnt < tx_num)
        {
        ...
        else
        {
            ...
            if (UART0_cb_event)
            {
            	UART0_cb_event(ARM_USART_EVENT_SEND_COMPLETE);
            }
        }
    }

    /* Receive interrupt */
    if ((status & LPUART_STAT_RDRF_MASK) && (crl & LPUART_CTRL_RIE_MASK))
    {
        ...
        if (rx_cnt >= rx_num)
        {
            ...
            if (UART0_cb_event)
            {
            	UART0_cb_event(ARM_USART_EVENT_RECEIVE_COMPLETE);
            }
        }
    }
}
